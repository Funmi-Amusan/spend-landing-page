...
" />
  ...
  >
    <div className='overflow-hidden w-full'>
      <motion.div
        initial={{ 
          x: slides[currentSlide].middleSection.reverseOrder ? '-100%' : '100%' 
        }}
        animate={{ x: 0 }}
        transition={{ 
          duration: 0.8, 
        ...
        className={`flex w-full justify-between items-center ${slides[currentSlide].middleSection.reverseOrder ? 'flex-row-reverse' : ''}`}
      >
        <img src={slides[currentSlide].middleSection.flag.src} alt="Flag" className='w-12 h-12' />
        <h2 className='text-xl lg:text-2xl text-white font-bold font-paytone'>{slides[currentSlide].middleSection.amount}</h2>
      </motion.div>
    </div>
  ...
  >
    <div className='h-fit max-w-[260px]'>
      ...
      {slides[currentSlide].middleSection.reverseOrder ? (
        ...
        <h3 className="text-2xl font-sans">{slides[currentSlide].rightSection.country}</h3>
      )}
